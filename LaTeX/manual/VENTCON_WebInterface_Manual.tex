...
\fancyhead[R]{\textcolor{ventrexblue}{v1.0}}
\fancyfoot[C]{\thepage}
...
\begin{itemize}
    \item \textbf{Real-Time Monitoring} -- Pressure and PWM gauges with live chart (always visible)
    \item \textbf{Control Parameters} -- Setpoint and PID tuning (expanded by default)
    \item \textbf{Auxiliary Settings} -- Filter, PWM frequency/resolution (collapsed by default)
...
\subsection{PWM Output Gauge}

The \textbf{PWM Output} gauge shows:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Green line}: PWM output percentage
\end{itemize}

\noindent\textbf{Toggle the chart:} Use the ``Show Chart'' checkbox to hide/show the chart. Hiding the chart can improve performance on slower devices.
...
The \textbf{Setpoint} slider controls the target outlet pressure:
...
\textbf{Integral (Ki)} & Eliminates steady-state error. Higher values = faster error correction but may cause oscillation \\
\textbf{Derivative (Kd)} & Dampens oscillations. Higher values = more damping but may slow response \\
\bottomrule
\end{tabular}
...
Each slider (Setpoint, Kp, Ki, Kd) has a \textbf{gear icon} (\symbol{"2699}) next to its label. Tapping this icon opens a settings popup where you can customize:
...
\noindent This feature allows you to narrow the slider range for finer control, or expand it for wider adjustment ranges. Settings are saved automatically and persist across power cycles.

\subsection{System Parameters}

\subsubsection{Low Pass Filter (α)}
...
Sets the pulse-width modulation frequency for the valve driver:
\begin{itemize}
...
The \textbf{System Information} section displays:

\begin{itemize}
    \item \textbf{Network Status}: Connection indicator and WiFi information
\end{itemize}
...
\noindent To save settings permanently (so they persist after power cycling), use the serial command interface with the \texttt{save} command.
...
Settings lost after restart & Use the \texttt{save} serial command to persist settings. \\
\bottomrule
\end{tabular}
...
Customize slider range & Tap gear icon (\symbol{"2699}) next to label \\
