...
\begin{figure}[H]
    \centering
        ...
        \label{fig:terminal}
\end{figure}
The terminal block connections are numbered as follows (from left to right; see Figure~\ref{fig:terminal}).\hfill
...
\begin{tcolorbox}[blanker, borderline west={1pt}{0pt}{ventrexblue}, left=1em]
    ...
    \begin{enumerate}[leftmargin=6em]
        \item [Pos. \textbf{1)}]\fbox{\texttt{Sen}} Connect to the analog pressure signal output of the pressure sensor.
...
\begin{tcolorbox}[blanker, borderline west={1pt}{0pt}{ventrexblue}, left=1em]
    ...
    For the proportional valve supplied with this demo kit, a 12\,V source rated for at least 4\,A 
    is recommended.\footnote{The VenCon device, however, accepts any supply voltage in the range of 12--30\,V.}.
\end{tcolorbox}
...
\begin{tcolorbox}[blanker, borderline west={1pt}{0pt}{ventrexblue}, left=1em]
    ...
    It is recommend that users first familiarise themselves with sensor behaviour, browser interface, 
...
It is recommended that before the VenCon device is wired up to the proportional valve,users first connect to the device via WiFi and explore 
...
This allows, for example, a smartphone and a laptop to both access the web interface simultaneously.}. 

The VentCon device creates its own WiFi access point. No router or internet connection is needed or used.
...
\begin{minipage}[ht]{0.55\textwidth}
    \vspace{-12mm}
        \begin{enumerate}
            \item \textbf{Power on} the VentCon unit via your power supply (connector see Section~\ref{sec:wiring}).
...
A \texttt{Reset PID} button clears the controller's internal state, which is useful when the system is saturated or oscillating.
Finally, a \texttt{Reset to Default} button restores all user-adjustable settings to their factory values after confirmation.
...
Figure \ref{fig:overview} and \ref{fig:pid-block} illustrate the closed-loop control architecture.
