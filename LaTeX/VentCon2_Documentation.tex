...
+--------------------------+    | + emergencyShutdown():   |                 |
           ...
           +-------------------+---------------+                             |
                ...
                +--------------------------+    +--------------------------+
                | - controlSystem:         |    | + PRESSURE_MIN: float    |
                |   ControlSystem*         |    | + PRESSURE_MAX: float    |
                | - webHandler: WebHandler*|    | + PWM_FREQ_MIN: int      |
                | - controlTaskHandle: Task|    | + PWM_FREQ_MAX: int      |
                | - networkTaskHandle: Task|    | + SOLENOID_PIN: int      |
                | - taskRunning: bool      |    | + ANALOG_PRESS_PIN: int  |
                +--------------------------+    | + PWM_CHANNEL_MOSFET: int|
                | + createTasks(): bool    |    | + DEFAULT_SETPOINT: float|
                | + startTasks(): void     |    | + EMERGENCY_LIMIT: float |
                | + stopTasks(): void      |    | + VALVE_MIN_DUTY: float  |
                | + getTaskStatus(): bool  |    | + VALVE_MAX_DUTY: float  |
                | + monitorTasks(): void   |    +--------------------------+
                +--------------------------+                    ^
                               |                                |
                               |          +---------------------+
                               |          |  All classes use
                               v          |  Constants namespace
                +-------------------------------------------------------------------+
                |                        main.cpp                                   |
                +-------------------------------------------------------------------+
...
• Singleton Pattern: Constants namespace provides global configuration
        ...
        *pressureInput = sensorManager->getPressure();
        ...
        // PID calculation and PWM output (only if not in auto-tune mode)
            ...
            *pwmOutput = constrain(*pwmOutput, 0, *pwm_max_value); 
            ...
            // Anti-windup and hysteresis compensation
            if (settings->antiWindup || settings->hysteresis) {
                handleAdvancedControl(previousOutput);
            }
...
\midrule
Control Task & Core 0 & High & PID control, sensor reading \\
Network Task & Core 1 & Medium & Web server, WiFi management \\
...
Sensor Reading & 1000 Hz & < 0.5 ms \\
PID Computation & 100 Hz & 10 ms period \\
